...
class TensorMultiPoint(TensorBase):
    ...
    def show(self, ctx=None, cmap=None, classes=None, **kwargs):
        ...
        for (cls_idx, _, color, _) in classes:
            ...
            if len(pts) == 0:
                continue
            ctx.scatter(pts[:, 0], pts[:, 1], color=color,
...
def extract_predictions(lbl, pred, score_thresh=0.1, min_area=4):
    ...
    contours, hierarchy = cv2.findContours(pscore.max(0).astype(
        np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    pboxes = [cv2.boundingRect(cnt)
              for cnt in contours if cv2.contourArea(cnt) >= min_area]
